...
#if defined(NDEBUG)
# error "BITCOINx2 cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BITCOINx2 Signed Message:\n";
...
{
    // BITCOINx2: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BITCOINx2: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BITCOINx2: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Welcome to BITCOINx2 Digital Currency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0474f007952809bb8a3db5889acaee95144de1a6eff931830479430096ffdcb09c3e786854b2d38bf3e91dac281ac2f7c1f696cfe9a497c7b159d9b7de4fb0219d") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0x42, 0x49, 0x54, 0x32 }; // BITCOINx2: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BITCOINx2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BITCOINx2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BITCOINx2Miner(CWallet *pwallet)
{
    printf("BITCOINx2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinx2-miner");
    ...
    try { loop {
        ...
        printf("Running BITCOINx2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BITCOINx2Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BITCOINx2Miner, pwallet));
}
