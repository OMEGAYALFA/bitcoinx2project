...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("BITCOINx2 version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  bitcoinx2d [options]                     " + "\n" +
                  "  bitcoinx2d [options] <command> [params]  " + _("Send command to -server or bitcoinx2d") + "\n" +
                  "  bitcoinx2d [options] help                " + _("List commands") + "\n" +
                  "  bitcoinx2d [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bitcoinx2:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: bitcoinx2.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: bitcoinx2d.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 14650 or testnet: 24650)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 14550 or testnet: 24550)") + "\n" +
        ...
        "\n" + _("SSL options: (see the BITCOINx2 Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BITCOINx2 is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("BITCOINx2 version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "BITCOINx2 server starting\n");
    ...
    if (fDisableWallet) {
        ...
        {
            if (nLoadWalletRet == DB_CORBITX2T)
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BITCOINx2") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BITCOINx2 to complete") << "\n";
